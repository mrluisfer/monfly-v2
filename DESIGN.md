...
- **One fact per column.** A table column holds one thing — the category, the
  ...
  time. Columns are 24 px apart and flush at the two outer edges (`px-3` on
  every cell, `first:pl-0 last:pr-0`; the lists carry it as `gap-x-6`), and room
  the columns can't give up becomes a sideways scroll — the ledger holds
  `min-w-[52rem]` and scrolls under it rather than crushing a description
  against an orb.
...
| Make room       | Back button's wrapper; Today and the next-month arrow on transactions | Svelte `slide` on x, 350 ms `quintOut`, so neighbours glide instead of jumping. The collapsing wrapper carries the gap as well: a flex `gap` outlives the control and snaps shut on unmount.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Category bars   | `CategoryBars`                                                        | Each bar grows from the left to its share of the largest (CSS width, 0.7 s, ease-out-quint, 60 ms apart, from `@starting-style`). Still under reduced motion.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
