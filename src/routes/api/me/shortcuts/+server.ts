...
import { isShortcutId } from '$lib/shortcuts';
 ...
 * takes it away with `"pinned": false`. Overview's lock lives on the page,
 * not here. Answers with every pinned id.
 */
export const PATCH: RequestHandler = async ({ locals, request }) => {
	...
	const { id, pinned } = (typeof body === 'object' && body !== null ? body : {}) as Record<
		string,
		unknown
	>;
	...
	return json(await setShortcutPinned(db, profile.id, id, pinned), { headers: NO_STORE });
};
