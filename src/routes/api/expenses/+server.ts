...
import { getExpenses } from '$lib/server/finance';
import type { RequestHandler } from './$types';

/**
 * What the signed-in user spent, bucketed for the chart as `/api/income` is:
...
export const GET: RequestHandler = async ({ locals, url }) => {
	...
	return json(expenses, { headers: { 'cache-control': 'private, no-store' } });
};
