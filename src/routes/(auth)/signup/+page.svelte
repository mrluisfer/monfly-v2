<script lang="ts">
	...
	onMount(() => {
		...
		try {
			tl.fromTo(
				q('[data-anim="panel"], [data-anim="card"]'),
				{ autoAlpha: 0, y: 24 },
				...
				.fromTo(
					q('[data-anim="lede"], [data-anim="promise"]'),
					{ autoAlpha: 0, y: 12 },
				...
				.fromTo(
					...
					{ autoAlpha: 1, y: 0, rotate: 0, scale: 1, duration: 0.7, stagger: 0.09 },
					'-=0.35'
				)
				...
				.fromTo(
					...
					{ autoAlpha: 1, scale: 1, duration: 0.5, stagger: 0.06, ease: 'back.out(1.8)' },
					'-=0.25'
				)
...
<div bind:this={root} class="grid flex-1 gap-4 lg:grid-cols-[1.15fr_1fr]">
	...
	>
		<div
			aria-hidden="true"
			class="pointer-events-none absolute -right-28 -bottom-36 size-[30rem] opacity-60"
		>
			<Blob color="violet" blur={40} spread={70} />
		</div>
		<div
			aria-hidden="true"
			class="pointer-events-none absolute -top-28 -left-20 size-80 opacity-50"
		>
		...
		<p
			class="relative mt-auto font-display text-5xl leading-[1.04] font-light tracking-tight text-balance @2xl:text-6xl"
		>
		...
		<p
			data-anim="lede"
			class="relative mt-4 max-w-md text-[0.9375rem] leading-relaxed text-fg-muted"
		>
		...
		<div class="relative mt-10 grid gap-3 @xl:grid-cols-[1.35fr_1fr]">
			<div data-anim="tile" class="rounded-[1.25rem] bg-sunken/85 p-5 pt-6 backdrop-blur-sm">
				<div class="mb-5 flex items-baseline justify-between gap-3">
					...
					<span
						class="tabular font-display text-lg"
			...
			<div
				data-anim="tile"
				class="flex items-center gap-4 rounded-[1.25rem] bg-sunken/85 p-5 backdrop-blur-sm"
			>
				...
				<div class="min-w-0">
					...
					<p
						class="tabular font-display text-2xl font-light"
		...
		<ul class="relative mt-3 flex flex-wrap gap-2" aria-label="Example categories">
			{#each chips as chip (chip.label)}
				<li
					data-anim="chip"
					class="inline-flex items-center gap-2 rounded-full border border-hairline bg-card px-3 py-1.5 text-sm"
				>
	...
	<main class="flex">
		<Card class="@container flex w-full flex-col p-8 sm:p-10 xl:p-12" data-anim="card">
			...
			<div class="mt-auto max-w-md pt-16">
				<h1
					class="font-display text-[2.5rem] leading-[1.05] font-light tracking-tight text-balance @lg:text-5xl"
				>
					Start with Monfly
				</h1>
				...
				<div aria-live="polite">
					...
					{#if !data.auth0Ready}
						<p class="mt-5 rounded-xl bg-sunken p-3 text-xs leading-relaxed text-fg-muted">
							Auth0 isn't configured yet — fill the <code>AUTH0_*</code> variables in
							<code>.env</code>
